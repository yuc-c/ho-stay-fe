<template>
    ...
    <h2 class="page-title">文章類別維護</h2>

    <div class="d-flex">

        <!-- 左：新增分類 -->
        <div style="width: 280px;">
            <h5 class="fw-bold mb-2">新增問題分類：</h5>
        ...
        <div class="flex-grow-1 ms-5 category-panel">
            <h5 class="fw-bold mb-2">類別列表：</h5>

            <div class="card p-4">

                <draggable v-model="categories" item-key="categoryId" handle=".drag-handle" animation="200"
                    @end="saveSort">
                    <template #item="{ element }">
                        <div class="category-item" @click="openEditModal(element)">
                            ...
                            <!-- 右側：拖曳手把 -->
                            <i class="bi bi-list drag-handle"></i>
    ...
    <div v-if="showModal" class="modal-mask" @click.self="closeModal">
        <div class="modal-container">
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
                ...
                <button class="btn btn-outline-dark btn-sm" @click="updateCategory">
                    更新
                </button>
            </div>
        </div>
...
async function saveSort() {
    const payload = categories.value.map((c, idx) => ({
        id: c.categoryId,
        sortOrder: idx + 1
    }))

    await axios.put('/api/questions/category/sort', payload, {
        headers: {
            "userId": adminUserId
        }
    })
...
async function createCategory() {
    ...
    if (!form.value.name) errors.value.name = "請輸入分類名稱"
    if (!form.value.description) errors.value.description = "請輸入分類說明"

    if (errors.value.name || errors.value.description) return

    await axios.post('/api/questions/category', {
...
async function openEditModal(category) {
    ...
    // 檢查是否可刪除
    const res = await axios.get(`/api/questions/category/check/${category.categoryId}`)
...
async function updateCategory() {
    ...
    if (!editForm.value.name) errorsEdit.value.name = "請輸入分類名稱"
    if (!editForm.value.description) errorsEdit.value.description = "請輸入分類說明"

    if (errorsEdit.value.name || errorsEdit.value.description) return

    await axios.put(`/api/questions/category/${editForm.value.id}`, {
...
async function confirmDelete() {
    ...
    await axios.delete(`/api/questions/category/${id}`, {
        headers: {
            "userId": adminUserId
        }
    })
